...
- [**pricing-anchor**](../../.claude/skills/pricing-anchor/SKILL.md) — 任何报价/评审动作前强制先锚市场价 level:定价格族 → peer 锚价 → 只调可量化差 → 才 propose。触发:任何交易标的的 propose / 评审 / 报价,或用户问「能做到多少 / 该报什么 / 这价合理吗」。
- [**expert-blindtest**](../../.claude/skills/expert-blindtest/SKILL.md) — 带专家换算值的数据出现时自动跑盲测循环:先盲算(不看专家数)→ 对比偏差 → 归因 → 录标定点 → 偏差超阈排查出规则。触发:用户贴的数据行里出现专家 equivalent / 换算值标注。
